...
#define MIN_TEXTURE_SIZE 32
	...
	if (DefaultTextureMap<=0 || DefaultLightMap<=0 || DefaultFogMap<=0)
		appErrorf("SGL: Failed to load default texture(s).");
								   ...
								   USGLCacheObject *CacheObject,BOOL AttemptingReloadAtFullSize)
{
	...
	if (!(Flags & (SF_LightMap | SF_FogMap)))
	{
		...
		if (MipMapped || (Texture.TextureFlags & TF_Realtime))
								  ...
								  USGLTexInfo &Info)
{
	...
	else 
	{
		...
		if (Texture.TextureFlags & TF_RealtimeChanged ||
			((Flags&(SF_LightMap|SF_FogMap)) && (Texture.MaxColor->D==0xffffffff)) )
		{
	...
	{
		debugf(NAME_Log,"SGL::SetTexture - Load failed. CacheId=0x%08X%08X, flags=0x%08X",
