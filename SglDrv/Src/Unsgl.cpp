/*=============================================================================
	...
	Based on code Copyright 1997 Epic MegaGames, Inc.
	This software is a trade secret.
	Compiled with Visual C++ 5.0. Best viewed with Tabs=4.
...
//*****************************************************************************
void USGLRenderDevice::InternalClassInitializer( UClass* Class )
{
	guard(USGLRenderDevice::InternalClassInitializer);
	if( appStricmp( Class->GetName(), "SGLRenderDevice" )==0 )
	{
		UEnum* ColorDepth=new(Class,"Color Depth")UEnum(NULL);
		new(ColorDepth->Names)FName( "16" );
		new(ColorDepth->Names)FName( "24" );

		UEnum* TextureDetailBias=new(Class,"Texture Detail Bias")UEnum(NULL);
		new(TextureDetailBias->Names)FName( "Near" );
		new(TextureDetailBias->Names)FName( "Far" );

		new(Class,"VertexLighting",    RF_Public)UBoolProperty( CPP_PROPERTY(VertexLighting    ),"Options",CPF_Config);
		new(Class,"FastUglyRefresh",   RF_Public)UBoolProperty( CPP_PROPERTY(FastUglyRefresh   ),"Options",CPF_Config);
		new(Class,"ColorDepth",        RF_Public)UByteProperty( CPP_PROPERTY(ColorDepth        ),"Options",CPF_Config,ColorDepth);
		new(Class,"TextureDetailBias", RF_Public)UByteProperty( CPP_PROPERTY(TextureDetailBias ),"Options",CPF_Config,TextureDetailBias);
	}
	unguard;
}
...
// Initializes SGL.
//*****************************************************************************
UBOOL USGLRenderDevice::Init( UViewport* InViewport )
{
	...
	debugf(NAME_Init,"SGL: Initializing...");
	...
	{
		// No good.
		debugf(NAME_Init,"SGL: Failed load the SGL library.");
		return 0;
	}
	...
	{
		// No good.
		debugf(NAME_Init,"SGL: Failed to create screen device (%ix%i) (%s)",
...
//todo: [USGLRenderDevice::Init] Initialize any other variables etc.
	...
	// Success.
    Viewport->MakeFullscreen( X, Y, 1 );
    Viewport->SetDrag( 1 );
    Viewport->SetMouseCapture( 1, 1 );

	debugf(NAME_Init,"SGL: Initialized.");
...
void USGLRenderDevice::Exit()
{
	...
	debugf(NAME_Exit,"SGL: Shutting down...");
	...
	debugf(NAME_Exit,"SGL: Terminated.");
...
void USGLRenderDevice::Flush()
{
	...
	debugf(NAME_Log,"Brightness=%f, Gamma=%f",Viewport->Client->Brightness,0.5 + 1.5*Viewport->Client->Brightness);
...
void USGLRenderDevice::Unlock( UBOOL Blit )
{
	...
	if( FlashScale!=FVector(.5,.5,.5) || FlashFog!=FVector(0,0,0) )
	{
		...
		// Setup color.
		SGLColor Color = SGLColor(FPlane(FlashFog.R,FlashFog.G,FlashFog.B,Min(FlashScale.R*2.f,1.f)));
...
// Get stats.
//*****************************************************************************
void USGLRenderDevice::GetStats( char* Result )
{
	...
	if (LastStats.DisplayCount==0)
	{
		...
		(
				Result,
				"startofframe=(%04.1f) renderfinish=(%04.1f) render=(%04.1f)",
	...
	else if (LastStats.DisplayCount==1)
	{
		...
		(
				Result,
				"csurfs=%03i (%04.1f) cpolys=%03i gpolys=%03i (%04.1f) tiles=%03i (%04.1f)",
	...
	else if (LastStats.DisplayCount==2)
	{
		...
		(
				Result,
				"texloads=(%04.1f) pal=%03i (%04.1f) tex=%03i (%04.1f) lmap=%03i (%04.1f) fmap=%03i (%04.1f) reload=%03i (%04.1f) lmaxcolor=%03i (%04.1f) fmaxcolor=%03i (%04.1f) \n",
	...
	else if (LastStats.DisplayCount==3)
	{
		...
		(
				Result,
				"LM Max U/V=%03i,%03i, LM Max Dim=%03i",
...
// Execute a command.
//*****************************************************************************
int USGLRenderDevice::Exec( const char *Cmd, FOutputDevice* Out )
{
	guard(USGLRenderDevice::Exec);
	if( ParseCommand(&Cmd,"GetRes") )
	{
		Out->Logf( "320x200 512x384 640x480 800x600 1024x768" );
		return 1;
	}
	else if( ParseCommand(&Cmd,"SetRes") )
	{
		...
		if( X && Y )
		{
			...
			int Status=DDCCreateRenderingObject(reinterpret_cast<HWND>(Viewport->GetWindow()),
												...
												(FastUglyRefresh)?1:3);
			if (Status!=DDCERR_OK)
				appErrorf("SGL: Failed to change resolution");
			unguard;
...
{ 
	...
	if (ddrval!=DD_OK)
	{
		debugf(NAME_Log,"SGL: ERROR: Failed to lock frame buffer for screenshot.");
		return;
	}
...
void USGLRenderDevice::Draw2DPoint( FSceneNode* Frame, FPlane Color, DWORD LineFlags, FLOAT X1, FLOAT Y1, FLOAT X2, FLOAT Y2 )
{
...
)
{
	...
	if (Surface.Texture)
	{
		...
		DWORD TextureFlags =	
			...
			((Surface.Texture->TextureFlags&TF_Realtime) ? SF_NoScale                      : 0);
